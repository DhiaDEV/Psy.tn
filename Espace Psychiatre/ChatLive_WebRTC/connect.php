...
<head>
    ...
    <script
        ...
        <script type="text/javascript"> 
            const conn = new WebSocket('ws://localhost:8888/?token=<?php echo $userObj->sessionID ; ?>');
        </script>
    ...
    <div class="pop-up flex justify-between w-96 bg-white rounded overflow-hidden">
          <div class="pl-6 border-green-600 px-2 py-2 flex items-center">
            ...
            <div class="flex flex-col">
                ...
                <div class="animate-pulse mx-2 text-xs font-200 relative flex">
                    <span class="flex">Appel vidéo</span> 
    ...
    <div class="w-2/5 border-r">
        <div class="h-full overflow-hidden">
            <div class="flex justify-between items-center border-b">
                <div class="mx-2 flex items-center justify-center">
                    <div class="flex-shrink-0 mx-2 my-4 rounded-full overflow-hidden h-12 w-12  cursor-pointer">
                        <img  class="w-full h-auto rounded-full select-none" src="../images/profileImage/<?php echo  $user->PhotoProfile;?>">
                    </div>
                    <div>
                        <span class="font-medium select-none"><?php echo $user->Prenom . ' ' . $user->Nom; ?></span>
                    </div>
                </div>
                <div class="flex items-center mx-4">
                    ...
                    <span class="menu-logout relative select-none transition hover:text-gray-500 mx-3 text-gray-600 cursor-pointer"><i class="fas fa-ellipsis-v"></i>
                        <div class="logout flex items-center justify-center rounded absolute right-0 top-2 bg-white border hover:bg-gray-200" style="width: 100px;height: 60px;">
                            <a href="../doctor-dashboard.php?userID=<?php echo $user->userID ?>" class="p-2 px-2 text-lg text-gray-600">
                            Quitter
                            </a>
                        </div>
                    </span>
            ...
            <div class="px-6 py-5 select-none">
                ...
                <input class="p-2 w-full rounded border" type="text" name="patientrech" placeholder="Rechercher patients ">
                <button type="submit" class="bttn">Rechercher</button>
                </form>
             </div>
             <div class="select-none overflow-hidden overflow-y-auto h-2/4">
                ...
                <!-- USER-LIST -->
                <?php 
                    $search_query = isset($_GET['patientrech']) ? $_GET['patientrech'] : '';
                    $userObj->getUser($search_query); 
                ?>
                </ul>
            </div>
        </div>
...
<div id="profile" class="flex flex-1 justify-center items-center">
    <div class="flex flex-col flex-1 h-full overflow-hidden overflow-y-auto items-center justify-center">
        <div class="w-60 h-60 right-img rounded-full overflow-hidden">
            <img class="h-auto w-full" src="\Psy.tn\Espace Patients\img/<?php echo $profileData->PhotoProfile;?>">
        </div>
        <div class="right-heading">
            <h2 class="text-center"><?php echo $profileData->Prenom .' '. $profileData->Nom; ?></h2>
