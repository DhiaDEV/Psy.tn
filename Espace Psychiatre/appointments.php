						...
						<ul class="main-nav">
							...
							<li class="has-submenu active">
								...
								<ul class="submenu">
									...
									<li class="active"><a href="appointments.php?userID=<?= $psyInfo['userID'] ?>">Rendez-Vous En Ligne</a></li>
						...
						<div class="col-md-12 col-12">
							<nav aria-label="breadcrumb" class="page-breadcrumb">
								<ol class="breadcrumb">
									...
									<li class="breadcrumb-item active" aria-current="page">Rendez-Vous En Ligne</li>
								</ol>
							</nav>
							<h2 class="breadcrumb-title">Rendez-Vous En Ligne</h2>
						</div>
					</div>
				</div>
			</div>
				...
				<div class="container-fluid">
				   ...
				   Nous nous excusons pour tout inconvénient que cela pourrait causer. Merci de votre compréhension.";
									...
									if (isset($_POST['recherche']) && !empty(trim($_POST['recherche']))) {
										...
										$query = 'SELECT * FROM compte_utilisateurs u JOIN rendez_vous r ON u.userID = r.IDPatient WHERE r.IDPsy = ? AND Statut="Nouveau" AND Type="En_ligne"';
										...
										$recupPatient = $bdd->prepare('SELECT * FROM compte_utilisateurs u JOIN rendez_vous r ON u.userID = r.IDPatient WHERE r.IDPsy = ? AND Statut="Nouveau" AND Type="En_ligne" ORDER BY IDRdv desc');
										$recupPatient->execute(array($getid));
									}
												...
												<div class="profile-info-widget">
													...
													<div class="appointment-action" style="padding-left: 60px;">
														<?php if ($Recrdv['Confirmer'] == 0) { ?>
															<a href="confirmerPatient.php?userID=<?php echo $psyInfo['userID']; ?>&confirmer=<?= $Recrdv['IDRdv'] ?>" class="btn btn-sm bg-success-light">
																<i class="fas fa-check"></i> Confirmer
															</a>
														<?php } ?>
														<a href="modifierDate.php?userID=<?php echo $psyInfo['userID']; ?>&modifier=<?= $Recrdv['IDRdv'] ?>" class="btn btn-sm bg-warning-light">
															<i class="fas fa-calendar-check"></i> Modifier la Date
														</a>
														<a href="appointments.php?userID=<?php echo $psyInfo['userID']; ?>&refuse=<?= $Recrdv['IDRdv'] ?>" class="btn btn-sm bg-primary-light">
															<i class="far fa-calendar-times"></i> Refuser La Confirmation
														</a>
														<a href="#" class="btn btn-sm bg-danger-light" onclick="afficherConfirmation(<?= $Recrdv['IDRdv'] ?>)">
														...
														<script>
															...
															function supprimer(idRDV) {
																...
																window.location.href = "supprimerRDV.php?userID=<?php echo $psyInfo['userID']; ?>&supprimer=" + idRDV;
