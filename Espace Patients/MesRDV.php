...
?>
    ...
    <head>
        <meta charset="utf-8">
        <title>Psy.tn: Mes Rendez-vous</title>
    ...
    <body>
        ...
        <div class="container-fluid bg-primary py-5 bg-header" style="margin-bottom: 90px;">
            <div class="row py-5">
                <div class="col-12 pt-lg-5 mt-lg-5 text-center">
                    <h1 class="display-4 text-white animated zoomIn">Mes Rendez-vous</h1>
                    <a href="profile.php?userID=<?= $patientInfo['userID'] ?>" class="h5 text-white">Accueil</a>
                    <i class="far fa-circle text-white px-2"></i>
                    <a href="MesRDV.php?userID=<?= $patientInfo['userID'] ?>" class="h5 text-white">Mes Rendez-vous</a>
                </div>
            </div>
        </div>
        </div>
        ...
        <div class="container">
            <table class="table table-success table-striped">
                <h2 style="text-align: center;">Gérer Mes Rendez-vous</h2>
                <br>
                <tr>
                    <th>Date/Heure</th>
                    <th>Patient</th>
                ...
                <?php
                $recupRDV = $bdd->prepare('SELECT r.IDRdv, r.IDPsy,r.paye,  r.Date, r.Time, p.Nom AS  PatientNom, p.Prenom AS PatientPrenom, ps.Nom AS PsyNom, ps.Prenom AS PsyPrenom
                FROM rendez_vous r
                JOIN compte_utilisateurs p ON r.IDPatient = p.userID
                JOIN compte_utilisateurs ps ON r.IDPsy = ps.userID
                WHERE r.Confirmer = 1 AND p.userID = ?
                ORDER BY r.IDRdv DESC');
                $recupRDV->execute(array($getid));

                if ($recupRDV->rowCount() > 0) {
                    while ($infoRDV = $recupRDV->fetch()) {


                ?>
                        <tr>
                            <td><?php echo $infoRDV['Date'] . ' /' . $infoRDV['Time'] ?></td>
                            <td><?php echo $infoRDV['PatientPrenom'] . ' ' . $infoRDV['PatientNom'] ?></td>
                            <td><?php echo $infoRDV['PsyPrenom'] . ' ' . $infoRDV['PsyNom'] ?></td>
                            <td>
                                <?php if ($infoRDV['paye'] != "succès") { ?>

                                    <a style="border-radius: 25px;" href="#" class="btn btn-danger" onclick="afficherConfirmation(<?php echo $infoRDV['IDRdv'] ?>)">Supprimer</a>
                                    <script>
                                        ...
                                        function supprimer(idRDV) {
                                            ...
                                            window.location.href = "supprimerRDV.php?userID=<?php echo $patientInfo['userID'] ?>&IDRdv=" + idRDV;
                                    ...
                                    <a style="border-radius: 25px;" href="./Stripe/tarification.php?userID=<?php echo $patientInfo['userID'] ?>&IDPsy=<?php echo $infoRDV['IDPsy'] ?>&IDRdv=<?php echo $infoRDV['IDRdv'] ?>" class="btn btn-success">Paiement</a>
                                <?php } ?>
                                <?php if ($infoRDV['paye'] == "succès") { ?>
                                    <button style="border-radius: 25px;" class="btn btn-dark">Patient</button>
                                <?php } ?>
